...
$app->get('/', function (Request $request, Response $response, $args) {
    ...
    $templateIndex = $twig->load("index.html");
    $page = $templateIndex->render($renderVars);
...
$app->get(
    "/entur", function (Request $request, Response $response, $args) {
        $payload = Entur::Get();
        $response->getBody()->write(json_encode($payload));
        return $response->withHeader('Content-Type', 'application/json');
    }
...
// Return power prices
$app->get(
    "/powerprices", function (Request $request, Response $response, $args) {
        try {
            $year = date("Y", time());
            $month = date("m", time());
            $day = date("d", time());
